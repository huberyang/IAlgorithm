package cn.ys.codility.lesson1.iterations;

public class BiggestGapInBinary {
    
//    A binary gap within a positive integer N is any maximal sequence of consecutive zeros that is surrounded by ones at both ends in the binary representation of N.
//        For example, number 9 has binary representation 1001 and contains a binary gap of length 
//        2. The number 529 has binary representation 1000010001 and contains two binary gaps: one of length 4 and one of length 
...
//    N is an integer within the range [1..2,147,483,647].
    ...
    public int solution(int N) {
        ...
        for(int i=0;i<charArray.length;i++){
            if(charArray[i]=='1' && flag==false){
                last = i;
                flag=true;
            }

            if(charArray[i] == '1' && flag == true){
                if((i-last)>biggestGap){
                    biggestGap = (i-last-1);
                }
                last = i;
            }
